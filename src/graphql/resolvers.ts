...
/*
        ...
        addVideoGameToMyList: async (_, {VideoGameID}: { VideoGameID: string}, {user}) => {
            if (!user) throw new Error("Por aquí no Puri, logeate");

            const db = getDb();

            const VideoGameToAdd= await db.collection(COLLECTION).findOne({_id: new ObjectId(VideoGameID)});

            if(!VideoGameToAdd) throw new Error("Te lo inventaste");

            await db.collection(COLLETION_USERS).updateOne(
                {_id: user._id},
                { $addToSet: { myVideoGames: VideoGameToAdd._id}}
            );

            const updatedUser = await db.collection(COLLETION_USERS).findOne({ _id: user._id})

            if(!updatedUser) throw new Error("Usuario no encontrado después de la ")
